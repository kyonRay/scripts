...
start_chain()
{
    ...
    cp nodes-signPackage/127.0.0.1/sdk/* ./web3sdk-noParallel-buildOfficial/dist/conf/
...
start_java(){
    if [  $PRECOMPILE = true ]; then
        ...
        END=$(date "+%H:%M:%S")
        bash get_tps.sh nodes-signPackage/127.0.0.1/node0/log/log_2*.log $START $END > tps_report
    else
        echo "solidity test"
        #userAdd
